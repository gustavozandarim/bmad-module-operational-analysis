---
name: "oam-setup"
description: Sets up Operational Analysis Method module in a project. Use when the user requests to 'install oam module', 'configure Operational Analysis Method', or 'setup Operational Analysis Method'.
---
...
It also **scaffolds the shared memory tree** at `{project-root}/_bmad/memory/oam/` and **installs the module's immutable law** there — the Analytical Integrity Constitution and the module conventions — which every OAM agent loads on activation. On a fresh, profile-less install it hands off to **Owen** for first-run onboarding.
...
3. Check for per-module configuration at `{project-root}/_bmad/oam/config.yaml` and `{project-root}/_bmad/core/config.yaml`. If either file exists:
...
python3 ./scripts/merge-help-csv.py --target "{project-root}/_bmad/module-help.csv" --source ./assets/module-help.csv --legacy-dir "{project-root}/_bmad" --module-code oam
```
...
## Scaffold OAM Memory & Install Module Law

OAM keeps its immutable law **and** its runtime memory under `{project-root}/_bmad/memory/oam/` — a persistent location the legacy cleanup below never touches. Resolve `{project-root}` to the real path for these filesystem operations:

1. **Create the tree if absent:** `{project-root}/_bmad/memory/oam/`, `…/daily/`, `…/profiles/`.
2. **Install the law (overwrite — it is immutable and versioned with the module):** copy `./assets/analytical-integrity-constitution.md` and `./assets/module-conventions.md` into `{project-root}/_bmad/memory/oam/`. Overwriting is correct on update; it never touches `index.md`, `lessons.md`, `daily/`, or `profiles/`.
3. **Seed orientation only if absent (never overwrite — these accumulate state):**
   - `index.md` — a short stub: module name, "profiles: none yet" (or list any found), and a pointer to the Constitution.
   - `lessons.md` — a `# OAM Method Lessons` header and nothing else.
...
```bash
python3 ./scripts/cleanup-legacy.py --bmad-dir "{project-root}/_bmad" --module-code oam --also-remove _config --skills-dir "{project-root}/.claude/skills"
```
...
After the greeting, count profiles under `{project-root}/_bmad/memory/oam/profiles/`. If **zero**, this is a community cold-start: tell the user the module is installed but has no profile yet, and hand off to **Owen** (`oam-agent-owen`) to run first-run onboarding — Owen builds the first `org-profile.md` plus a seed `taxonomy.md`, then routes the user into their first analysis. If one or more profiles already exist, skip onboarding.
